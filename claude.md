...
- 상태에 따라 내용이 바뀌는 영역(설명 박스, 라벨 행, 수식 줄)은 가장 긴 상태 기준으로 minHeight를 지정해 레이아웃이 흔들리지 않게 한다. (설명 박스는 모든 상태를 한 칸에 겹쳐 두고 `visibility`로 토글, 라벨 행은 `ExcelGrid`의 `reserveLabelRow`로 항상 자리를 차지하게 하는 방식도 같은 목적이다.)
