...
- 지시문에 "멈춘다"(또는 "멈추고 보고")가 있으면 **그 지점에서 멈추고 보고**한다. 다음 배치·다음 단계로 넘어가지 않는다.
- `git push`는 **사용자가 명시적으로 지시할 때만** 한다. 커밋도 **지시가 있을 때만** 한다. (이전의 "수정 후 자동 커밋·push" 관행은 이 규칙으로 대체한다.)
- 생존 규칙(`survivorRules.js`)·엔진(`src/excel-engine/`)·채점기(`calcGrader.js` 등) **동작 변경은 구현 전에 보고하고 승인**을 받는다. 데이터 템플릿 추가와 달리, 이들은 앱 채점·계산 결과에 영향을 줄 수 있다.
- 멈춘 지점에서 받은 입력이 **짧거나 뜻이 불분명하면**(예: "rr", "ㅇㅇ", 오타로 보이는 입력) 진행으로 해석하지 말고 **무엇을 할지 되묻는다.**
...
- 상태에 따라 내용이 바뀌는 영역(라벨 행, 설명 박스, 수식 줄)은 가장 큰 상태 기준으로 자리를 미리 확보해 레이아웃이 흔들리지 않게 한다. 예약된 빈 영역에는 테두리·배경을 넣지 않는다. (설명 박스는 모든 상태를 한 칸에 겹쳐 두고 `visibility`로 토글, 라벨 행은 `ExcelGrid`의 `reserveLabelRow`로 고정 height만 차지하게 하는 방식이 같은 목적이다.)
