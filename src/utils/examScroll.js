...
// 거슬러 올라가 overflow-y 가 auto/scroll 인 첫 요소를 찾아 scrollTop=0 하고, 추가로
// 루트를 scrollIntoView({block:"start"}) 로 맞춘다. 찾은 컨테이너를 반환한다(디버깅·보고용).
export function scrollExamTop(rootEl) {
  ...
  else { try { window.scrollTo(0, 0); } catch { /* 무시 */ } }
  try { rootEl?.scrollIntoView({ block: "start" }); } catch { /* 무시 */ }
  return container;
}
