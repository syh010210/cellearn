...
import { COPY } from "../../data/membership";
            ...
            return (
              <div key={i} style={{ borderTop: i ? `1px solid ${UI.line}` : "none" }}>
                <button
                  ...
                  style={{ width: "100%", textAlign: "left", background: isOpen ? UI.panelAlt : UI.surface, border: "none", padding: isOpen ? "18px 20px 0" : "18px 20px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, fontFamily: UI.font }}
                >
                ...
                {isOpen && <div id={`faq-panel-${i}`} role="region" style={{ background: UI.panelAlt, padding: "14px 20px", color: UI.mut, fontSize: 14, lineHeight: 1.75 }}>{it.a}</div>}
              </div>
            );
          })}
        </div>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 48, alignItems: "stretch", flexWrap: "wrap" }}>
          <div style={{ flex: "1 1 400px", minWidth: 300, display: "flex", flexDirection: "column" }}>
            ...
            {/* 무료 체험 바로가기 — 문단과 CTA 버튼 사이, 컴팩트한 가로 배너 */}
            <div
              onClick={onTrial}
              role="button"
              tabIndex={0}
      ...
      <section id="pricing" style={{ background: UI.greenDeep, color: "#fff", padding: "64px 24px", scrollMarginTop: 66 }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          <p style={{ textAlign: "center", color: "#d5dbd8", fontSize: 16.5, margin: "0 0 8px", lineHeight: 1.6 }}>
            단 한 번의 결제로 추가 결제 없이 전 차시, 실습, 복습, 실전 모드를 무제한으로.
          </p>
          <p style={{ textAlign: "center", color: UI.lime, fontSize: 15, fontWeight: 700, margin: "0 0 40px", lineHeight: 1.6 }}>
            {COPY.promoLabel}<br /><span style={{ color: "#d5dbd8", fontWeight: 500 }}>{COPY.promoSentence}</span>
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
            ...
            <div style={{ position: "relative", background: UI.teal, border: `1px solid ${tealLine}`, borderRadius: UI.rLg, padding: "28px 26px" }}>
              ...
              <div style={{ ...num, fontSize: 36 }}>₩70,000<span style={{ fontSize: 15, fontWeight: 500, color: UI.invMut }}> · 이용기간 2개월</span></div>
              <div style={{ margin: "20px 0" }}>
                {["7일 완성, 20차시 커리큘럼", "엑셀 파일 자동 채점 · 퀴즈 오답노트", "사전 점검 세션 · 누적 복습 시스템", "개념 학습 후 실전 문제 무제한 풀이"].map((f) => (
